...
function validateImageUrl(url) {
    ...
    try {
        ...
        const isAllowedDomain = allowedDomains.some(d => hostname.includes(d));
        return isAllowedDomain || validExtensions.test(pathLower);
    } catch (e) {
        return false;
    }
}
...
async function initializeCarousel() {
    ...
    let mostRecent = null;
    const sortedRecent = [...allAddons].sort((a, b) => new Date(b.publish_date) - new Date(a.publish_date));
    for (const addon of sortedRecent) {
    ...
    carouselSlides = [];
    if (mostDownloaded) carouselSlides.push({ type: 'mostDownloaded', addon: mostDownloaded, title: 'Más Descargado' });
    if (minecraftAddon) carouselSlides.push({ type: 'minecraft', addon: minecraftAddon, title: 'Minecraft Oficial' });
    if (mostRecent) carouselSlides.push({ type: 'mostRecent', addon: mostRecent, title: 'Más Reciente' });
    if (recommendationOfTheDay) carouselSlides.push({ type: 'recommendation', addon: recommendationOfTheDay, title: 'Recomendación del Día' });
    if (extraBanner) carouselSlides.push({ type: 'extra', addon: extraBanner, title: 'Destacado' });
...
function renderCarousel() {
    ...
    carouselSlides.forEach((slide, index) => {
        ...
        const bannerUrl = validateImageUrl(addon.banner_url) ? addon.banner_url : (validateImageUrl(addon.icon_url) ? addon.icon_url : 'img/default-banner.png');
        const iconUrl = validateImageUrl(addon.icon_url) ? addon.icon_url : bannerUrl;
        const safeTitle = escapeHtml(addon.title || '');
        const safeAuthor = escapeHtml(addon.author_name || 'Usuario');
        const safeDescription = addon.description ? escapeHtml(addon.description.substring(0, 120) + '...') : '';
        
        const slideElement = document.createElement('div');
        slideElement.className = 'carousel-slide';
        slideElement.style.backgroundImage = `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.7)), url('${bannerUrl}')`;
        slideElement.innerHTML = `
            <div class="slide-info">
                <img src="${iconUrl}" alt="${safeTitle}" class="slide-icon" onerror="this.src='img/default-addon.png'">
                <div class="slide-text">
                    <div class="slide-title">${safeTitle.length > 50 ? safeTitle.substring(0, 47) + '...' : safeTitle}</div>
